...
describe("Mean Radius Precision Srategy", () => {
    ...
    let target: Target;
    let precision: MeanRadiusPrecisionStrategy;

    beforeEach(() => {
        target = new Target([new TargetRing(0, 10, 2), new TargetRing(10, 20, 1)]);
        precision = new MeanRadiusPrecisionStrategy();
    });
    ...
    it("Can calculate the precision of 2 arrows", () => {
        ...
        expect(precision.getPrecision(target)).to.equal(Math.hypot(1, 1));
    });
    ...
    it("Can calculate the precision of 3 arrows", () => {
        ...
        expect(precision.getPrecision(target)).to.equal(Math.hypot(2, 2) * 2 / 3);
    });
